...
// Attempt to initialize Resend client at module load time
if (resendApiKey && typeof resendApiKey === 'string' && resendApiKey.startsWith('re_')) {
  ...
  // Key is present but invalid format
    ...
    if (emailError) {
      ...
      if (emailError.message) {
        if (emailError.message.includes('Invalid API Key')) {
            userFriendlyMessage = 'Failed to send message: The email service API key is invalid. Please verify your RESEND_API_KEY.';
            ...
            userFriendlyMessage = 'Failed to send message: The sender email address is not configured correctly (e.g., domain not verified in Resend dashboard).';
