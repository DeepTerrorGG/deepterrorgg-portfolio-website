...
const DownDetectorDetector: React.FC = () => {
  ...
  const runChecks = () => {
    ...
    const checkEndpoint = async (region: { name: string; endpoint: string }): Promise<RegionData> => {
      ...
      try {
        ...
        // and infer "up" status if the request doesn't throw an error.
        const response = await fetch(region.endpoint, { method: 'HEAD', mode: 'no-cors', cache: 'no-cache' });
  ...
  const StatusDisplay = () => {
    switch (status) {
      case 'checking':
        return (
          <div className="bg-yellow-500/10 border border-yellow-500/30 text-yellow-300 p-6 rounded-lg">
            <h2 className="font-bold text-xl flex items-center gap-2"><Loader2 className="animate-spin" /> CHECKING</h2>
            <p className="text-sm">Pinging downdetectorsdowndetector.com from multiple regions...</p>
          </div>
        );
      case 'ok':
        return (
          <div className="bg-green-600/20 border border-green-500/40 text-green-300 p-6 rounded-lg">
            <h2 className="font-bold text-xl flex items-center gap-2"><Wifi /> OK</h2>
      ...
      case 'down':
        return (
          <div className="bg-red-500/10 border border-red-500/30 text-red-300 p-6 rounded-lg">
            <h2 className="font-bold text-xl flex items-center gap-2"><CircleSlash /> DOWN</h2>
    ...
    <Table>
      <TableHeader>
        <TableRow className="border-border/20 hover:bg-transparent">
          <TableHead className="text-white/80">Region/Service</TableHead>
          <TableHead className="text-white/80">Status</TableHead>
          <TableHead className="text-white/80">HTTP</TableHead>
          <TableHead className="text-white/80 text-right">Latency</TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
        {data.map(region => (
          <TableRow key={region.name} className="border-border/20 hover:bg-white/5">
            ...
            <TableCell>
              {region.status === 'checking' ? <Loader2 className="w-4 h-4 animate-spin"/> : (
                <Badge variant='outline' className={cn(
                  ...
                  <span className={cn("mr-1.5 h-2 w-2 rounded-full", region.status === 'Up' ? 'bg-green-400' : 'bg-red-400')}></span>
                  {region.status}
                </Badge>
              )}
            </TableCell>
            <TableCell>{region.http ?? 'N/A'}</TableCell>
            <TableCell className="text-right">{region.latency ? `${region.latency} ms` : '...'}</TableCell>
          </TableRow>
        ))}
      </TableBody>
    </Table>
      ...
      <div className="w-full max-w-3xl mx-auto space-y-6">
        ...
        <Card className="bg-[#161b22] border-border/20">
          <CardContent className="p-6 space-y-4">
             <StatusDisplay />
             <div>
                <h3 className="font-semibold text-muted-foreground text-sm mb-2 px-4">CHECKS BY REGION/SERVICE</h3>
