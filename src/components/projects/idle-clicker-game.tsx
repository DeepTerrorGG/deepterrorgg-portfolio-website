...
const IdleClickerGame: React.FC = () => {
  ...
  const [maxCPSAchieved, setMaxCPSAchieved] = useState(0);
  ...
  useEffect(() => {
    ...
    // Check if player has a name, if not show modal
  ...
  // Track max CPS
  useEffect(() => {
    if (clicksPerSecond > maxCPSAchieved) {
      setMaxCPSAchieved(clicksPerSecond);
    }
  }, [clicksPerSecond, maxCPSAchieved]);
  ...
  const handleRebirth = () => {
    ...
    // Submit to leaderboard after rebirth
    if (db && playerName && playerId) {
      submitScore(db, 'idle-clicker', playerId, playerName, {
        totalClicks: clicks,
        rebirths: rebirthCount + 1,
        prestigePoints: prestigePoints + pointsToGain,
        maxCPS: maxCPSAchieved
  ...
  return (
    <div className="flex items-center justify-center w-full h-full bg-card p-4 sm:p-6 lg:p-8">
        ...
        <Card className="md:col-span-2 shadow-inner">
          <Tabs defaultValue="cps-upgrades" className="w-full h-full flex flex-col">
            <TabsList className="grid w-full grid-cols-5">
              ...
              <TabsTrigger value="leaderboard"><Award className="mr-1 h-4 w-4" />Leaders</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="leaderboard" className="flex-grow p-4">
              <LeaderboardPanel
                gameId="idle-clicker"
                currentPlayerId={playerId}
                metrics={[
                  { key: 'totalClicks', label: 'Total Clicks', format: formatters.abbreviated },
                  { key: 'rebirths', label: 'Rebirths', format: formatters.number },
                  { key: 'prestigePoints', label: 'Prestige', format: formatters.number },
                  { key: 'maxCPS', label: 'Max CPS', format: formatters.abbreviated }
                ]}
              />
            </TabsContent>
          </Tabs>
      ...
      <PlayerNameModal
        ...
        gameName="Idle Clicker Game"
        description="Enter your name to compete on the global leaderboard and track your clicking journey!"
      />
    </div>
  );
};
