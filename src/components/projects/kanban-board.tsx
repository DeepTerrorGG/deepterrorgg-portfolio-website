...
const TaskCard = ({ task, colId, onEdit }: { task: Task, colId: string, onEdit: (task: Task & {colId: string}) => void }) => {
    const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: task.id });
    const assignee = mockTeam.find(m => m.uid === task.assignedTo);
    const categoriesQuery = useMemoFirebase(() => query(collection(useFirestore(), 'kanban-categories')), []);
    const { data: categories } = useCollection<TaskCategory>(categoriesQuery);
    ...
    return (
        <div ref={setNodeRef} style={style} {...attributes} {...listeners}>
            <Card
                className={cn("p-3 shadow-sm border bg-card cursor-grab active:cursor-grabbing")}
...
const KanbanBoard: React.FC = () => {
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    const activeCol = findContainer(active.id as string);
    const overColId = over.data.current?.sortable?.containerId || (findContainer(over.id as string)?.id);
    if (!activeCol || !overColId || activeCol.id === overColId) return;
    ...
    setTasksByColumn(prev => {
        const newTasks = { ...prev };
        const activeTasks = newTasks[activeCol.id].filter(t => t.id !== active.id);
        const taskToMove = activeCol.tasks.find(t => t.id === active.id);
        if(!taskToMove) return prev;
        
        const overTasks = [...(newTasks[overCol.id] || [])];
        const overIndex = over.data.current?.sortable ? over.data.current.sortable.index : overTasks.length;
        
        overTasks.splice(overIndex, 0, taskToMove);

        newTasks[activeCol.id] = activeTasks;
        newTasks[overCol.id] = overTasks;
        return newTasks;
    });
  };
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const fromCol = findContainer(active.id as string);
    const toColId = over.data.current?.sortable?.containerId || findContainer(over.id as string)?.id;
    const toCol = columns.find(c => c.id === toColId);
    
    if(!fromCol || !toCol) return;

    const activeTask = fromCol.tasks.find(t => t.id === active.id);
    if(!activeTask) return;
    
    if (fromCol.id === toCol.id) {
        // Reordering within the same column
        const oldIndex = fromCol.tasks.findIndex(t => t.id === active.id);
        const newIndex = toCol.tasks.findIndex(t => t.id === over.id);
        if(oldIndex !== newIndex) {
            const reorderedTasks = arrayMove(fromCol.tasks, oldIndex, newIndex);
            const batch = writeBatch(firestore);
            reorderedTasks.forEach((task, index) => {
                const taskRef = doc(firestore, `kanban-columns/${fromCol.id}/tasks/${task.id}`);
        ...
        const fromColRef = collection(firestore, `kanban-columns/${fromCol.id}/tasks`);
        const toColRef = collection(firestore, `kanban-columns/${toCol.id}/tasks`);
        const { id, ...taskData } = activeTask;

        const batch = writeBatch(firestore);
        batch.delete(doc(fromColRef, id));
        batch.set(doc(toColRef, id), { ...taskData, order: toCol.tasks.length }); // Simplified order for now
        await batch.commit();
    }
  };
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd}>
        ...
        <ScrollArea className="flex-grow w-full" orientation="horizontal">
            ...
            {columns?.map(column => (
                <Card key={column.id} className="w-80 flex-shrink-0 flex flex-col max-h-full bg-muted/40">
                    ...
                    <CardContent className="p-3 space-y-3">
                         <SortableContext items={column.tasks.map(t => t.id)} strategy={verticalListSortingStrategy}>
                            {column.tasks?.map(task => (
                                <TaskCard key={task.id} task={task} colId={column.id} onEdit={setEditingTask}/>
                            ))}
                         </SortableContext>
                    </CardContent>
                </ScrollArea>
        ...
        <DragOverlay>
            {activeTask ? (
                <div className="w-80">
                     <TaskCard task={activeTask} colId="" onEdit={() => {}} />
                </div>
            ) : null}
        </DragOverlay>
        </div>
